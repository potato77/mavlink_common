...
void timerCallback(const ros::TimerEvent& e, Generic_Port *port)
{
...
int main(int argc, char **argv)
{
    ...
    nh.param<int>("udp_port", udp_port, 14540);
	...
	if(use_udp)
	{
		port = new UDP_Port(udp_ip_adr, udp_port);
	}
	else
	{
    ...
    ros::Timer timer = nh.createTimer(ros::Duration(1.0), boost::bind(&timerCallback,_1,port));
